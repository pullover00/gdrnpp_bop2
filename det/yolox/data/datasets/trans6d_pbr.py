...
class TRANS6D_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get("dataset_root", osp.join(DATASETS_ROOT, "BOP_DATASETS/trans6D/train_pbr"))
    #    self.xyz_root = data_cfg.get("xyz_root", osp.join(self.dataset_root, "xyz_crop"))
        ...
        self.models_root = data_cfg["models_root"]  # BOP_DATASETS/trans6D/models
    ...
    #    self.with_depth = data_cfg["with_depth"]
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.trans6D.id2obj.items() if obj_name in self.objs]
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(
                    self.models_root,
                    f"obj_{ref.trans6D.obj2id[obj_name]:06d}.ply",
                ),
...
def get_trans6d_metadata(obj_names, ref_key):
...
trans6d_model_root = "BOP_DATASETS/trans6D/models/"
################################################################################

SPLITS_TRANS6D_PBR = dict(
    trans6D_pbr_train=dict(
        name="trans6D_pbr_train",
        objs=ref.trans6D.objects,  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/trans6D/train_pbr"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/trans6D/models"),
       # xyz_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/trans6D/train_pbr/xyz_crop"),
...
# single obj splits
# for obj in ref.trans6D.objects:
#     for split in ["train"]:
#         name = "trans6D_pbr_{}_{}".format(obj, split)
#         if split in ["train"]:
#             filter_invalid = True
#         else:
#             raise ValueError("{}".format(split))
#         if name not in SPLITS_TRANS6D_PBR:
#             SPLITS_TRANS6D_PBR[name] = dict(
#                 name=name,
#                 objs=[obj],  # only this obj
#                 dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/trans6D/train_pbr"),
#                 models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/trans6D/models"),
#                 xyz_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/lmo_3r/train_pbr/xyz_crop"),
#                 scale_to_meter=0.001,
#                 with_masks=True,  # (load masks but may not use it)
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_TRANS6D_PBR:
        used_cfg = SPLITS_TRANS6D_PBR[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, TRANS6D_PBR_Dataset(used_cfg))
    ...
    MetadataCatalog.get(name).set(
        ...
        evaluator_type="bop",
        **get_trans6d_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )


def get_available_datasets():
    return list(SPLITS_TRANS6D_PBR.keys())
