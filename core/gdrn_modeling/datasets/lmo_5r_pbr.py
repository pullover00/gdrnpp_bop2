...
class LM_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get("dataset_root", osp.join(DATASETS_ROOT, "BOP_DATASETS/lmo_5r/train_pbr"))
...
SPLITS_LM_PBR = dict(
    lmo_5r_pbr_train=dict(
        name="lmo_5r_pbr_train",
        objs=LM_OCC_OBJECTS,  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/lmo_5r/train_pbr"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/lmo_5r/models"),
        xyz_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/lmo_5r/train_pbr/xyz_crop"),
...
for obj in ref.lmo_full.objects:
    for split in ["train"]:
        name = "lmo_5r_pbr_{}_{}".format(obj, split)
        if split in ["train"]:
            filter_invalid = True
        else:
